...
<template>
  <section class="w-full h-screen pt-28">
    <h1
      class="text-2xl font-bold leading-tight tracking-tight text-gray-900 md:text-4xl pl-2 md:pl-10"
    >
      <span class="text-primary">Candidates </span>Information
    </h1>
    <RouterLink to="ballot">
      <div
        class="fixed top-28 md:right-10 right-2 animate-bounce cursor-pointer bg-white py-2 px-4 outline outline-orange-500 hover:outline-primarybg-primary text-orange-700 font-bold z-20"
      >
        Vote Now
      </div>
    </RouterLink>
    ...
    <div
      v-for="(positionData, position) in groupedDataByPosition"
      :key="position"
      class="flex flex-wrap justify-center gap-10 p-10 w-full h-max pt-10 rounded-3xl"
    >
      <div
        v-for="item in positionData"
        :key="item.id"
        class="relative md:w-96 max-w-96 bg-white rounded-3xl shadow-xl md:mt-0 w-full xl:p-0"
      >
        <div class="pt-14 p-8">
          <div class="absolute left-0 top-0 w-2/5 mx-auto">
            <div
              class="bg-primary text-lg p-1 text-center text-white font-bold"
            >
              {{ item.position }}
            </div>
          </div>
          ...
          <div class="-space-y-2">
            ...
            <div class="text-lg p-0.5 text-center text-primary font-medium">
              {{ item.partylist }}
            </div>
          </div>
          ...
          <div class="w-full mx-auto">
            <div
              class="md:text-lg w-full p-0.5 text-center text-gray-500 font-bold"
            >
              Achievements/Credentials
            </div>
            <textarea
              class="w-full p-2 text-center h-24"
          ...
          <div class="w-full mx-auto">
            <div class="md:text-lg p-0.5 text-center text-gray-500 font-bold">
              Platforms/Goals
            </div>
            <textarea
              class="w-full p-2 text-center h-24"
